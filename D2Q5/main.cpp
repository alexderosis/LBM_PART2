...
vector<const double> wT = {1/3., 1/6., 1/6., 1/6., 1/6., 1/6.};
    ...
    for(int y=0; y<ny; y++)
		{
			...
			for(int k=0; k<np; k++)
			{
				...
				if(k<npT)
					g1[id*npT+k] = g2[id*npT+k] = wT[k]*T*(1. + 1./cs2*(U*cx[k]+V*cy[k]));
			}
		}
}
///----------------------------------------------------------------------------------------------------------------------------------
    ...
    for(int y=0; y<ny; y++)
	        ...
	        if(k<npT)
				...
				for(int k=0; k<np; k++)
				{
					...
					if(k<npT)
						g1[id*npT+k] = omegaT1*g1[id*npT+k] + omegaT*wT[k]*T*(1. + 1./cs2*(U*cx[k]+V*cy[k]));
					newx = x+cx[k];
					newy = y+cy[k];
					if(x==0 || x==nx-1)
						newx = (newx+nx)%nx;
...
void boundary_conditions()
{
	...
	for(int x=0; x<nx; x++)
		for(int k=0; k<np; k++)
		{
			...
			if(k<npT)
				g2[id*npT+k] = wT[k]*T_h;

			id = x*ny+ny-1;
			f2[id*np+k] = f1[id*np+opp[k]];
			if(k<npT)
				g2[id*npT+k] = wT[k]*T_l;
		}
}
///----------------------------------------------------------------------------------------------------------------------------------
